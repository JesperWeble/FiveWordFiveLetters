    ...
    public class FindWordsMethods
    {
        ...
        public static void RecursiveMethod(int[] words, int index, string result = "", int mask = 0)
        {
            ...
            if (mask == 0)
            {
                ...
                {
                    RecursiveMethod(words, x-1, ReadFile._dictionary[words[x]], words[x]);
                });
            ...
            else
            {
                for (int i = index; i >= 0; i--)
                {
                    if ((mask & words[i]) == 0)
                    {
                        RecursiveMethod(words, i - 1, string.Join(" ", result, ReadFile._dictionary[words[i]]).TrimStart(), mask | words[i]);
                    }
                }
            }
        }
